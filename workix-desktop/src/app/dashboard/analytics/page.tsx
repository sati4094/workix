...
function KPICard({ title, value, subtitle, trend, trendValue, icon, gradient, loading }: KPICardProps & { loading?: boolean }) {
  return (
    <div className={`relative overflow-hidden rounded-xl bg-gradient-to-br ${gradient} p-6 text-white shadow-lg transition-all hover:shadow-2xl hover:scale-105`}>
      ...
      <div className="relative z-10">
        ...
        <h3 className="text-3xl font-bold mb-1 min-h-[2.5rem]">
          {loading ? <div className="h-8 w-8 mx-auto animate-spin rounded-full border-t-2 border-b-2 border-white" /> : value}
        </h3>
...
export default function AnalyticsPage() {
  ...
  const { data: dashboardData, isLoading: dashboardLoading, isFetching: dashboardFetching } = useDashboardStats(timeRange);
  const { data: realTimeData, isLoading: realTimeLoading, isFetching: realTimeFetching } = useRealTimeMetrics();
  const { data: trendsData, isLoading: trendsLoading, isFetching: trendsFetching } = useAnalyticsTrends(timeRange);
  const { data: assetData, isLoading: assetLoading, isFetching: assetFetching } = useAssetReliability();
  const { data: techData, isLoading: techLoading, isFetching: techFetching } = useTechnicianPerformance();
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200">
            <h3 className="text-lg font-semibold mb-4 flex items-center">
              <Activity className="w-5 h-5 mr-2 text-blue-500" />Work Order Trends
            </h3>
            {(trendsLoading || trendsFetching) ? (
              <div className="flex items-center justify-center h-[300px]"><div className="h-10 w-10 animate-spin rounded-full border-t-2 border-b-2 border-blue-400 mb-2"></div></div>
            ) : (
              <ResponsiveContainer width="100%" height={300}>
                <AreaChart data={dashboard?.trends || []}>
                  ...
                  <XAxis dataKey="date" stroke="#9CA3AF" tick={{ fontSize: 12 }} tickFormatter={(value) => new Date(value).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} />
                  <YAxis stroke="#9CA3AF" tick={{ fontSize: 12 }} />
                  <Tooltip contentStyle={{ backgroundColor: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px' }} labelFormatter={(value) => new Date(value).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })} />
                  <Legend />
                  <Area type="monotone" dataKey="total" stroke={COLORS.primary} fillOpacity={1} fill="url(#colorTotal)" name="Total Work Orders" />
            ...
            {(dashboardLoading || dashboardFetching) ? (
              ...
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={costAnalysis}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                  <XAxis dataKey="week" stroke="#9CA3AF" tick={{ fontSize: 12 }} tickFormatter={(value) => new Date(value).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} />
                  <YAxis stroke="#9CA3AF" tick={{ fontSize: 12 }} />
                  <Tooltip contentStyle={{ backgroundColor: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px' }} formatter={(value: any) => `$${parseFloat(value).toFixed(2)}`} />
                  <Legend />
                  <Line type="monotone" dataKey="total_parts_cost" stroke={COLORS.success} strokeWidth={3} dot={{ fill: COLORS.success, r: 5 }} name="Total Cost" />
                  <Line type="monotone" dataKey="avg_cost_per_wo" stroke={COLORS.secondary} strokeWidth={2} dot={{ fill: COLORS.secondary, r: 4 }} name="Avg Cost per WO" />
                </LineChart>
              </ResponsiveContainer>
            )}
          </div>
